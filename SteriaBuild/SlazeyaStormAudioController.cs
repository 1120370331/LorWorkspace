...
public sealed class SlazeyaStormAudioController : IDisposable
{
    ...
    public const float GatherGain = 0.55f;
    ...
    {
        float onset = Mathf.Clamp01(elapsed / 0.03f);
        float growth = Mathf.Clamp01(elapsed / SlazeyaStormVisualController.GatherDuration);
        return 0.32f * onset * onset * (3f - 2f * onset)
            + 0.23f * growth * growth * (3f - 2f * growth);
    }
