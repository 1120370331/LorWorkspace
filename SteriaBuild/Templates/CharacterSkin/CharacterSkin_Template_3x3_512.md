...
> 此文件保留为历史整图版图模板。新角色/重制默认使用 [head / wearing 分层建模流程](../../../ModGuideDocs/12_战斗单位分层建模与核心书页投影.md)：部件可带参考生图或直接绘制，美术效果优先；生成使用便于处理的纯色底，装配导出后再排九宫格用于检查。
...
- 所有原始动作统一朝屏幕左侧；游戏会在单位朝右时自动水平翻转角色根节点
- 需要透明背景时，可用现有抠白脚本处理
...
Promopts:
  ...
  画风：2D Unity 角色，人物严格3头身，二次元赛璐璐+厚涂风格，大块填涂+细节处细线条+用亮色代表光源的光影效果                                         
  
  网格：3x3，每格 512x512，*人物角色占256x256*，按“行优先”顺序摆放                                                                                            
  动作顺序（从左到右、从上到下）：                                                                                                       
  1) Default  2) Guard  3) Evade                                                                                                         
  4) Damaged  5) Slash  6) Penetrate                                                                                                     
  7) Hit      8) Move   9) S1                                                                                                            
                                                                                                                                         
  统一方向：所有动作朝同一方向（必须默认朝左），视角一致；不要生成玩家/敌人两套方向，游戏会自动翻转
...
# 风格设定
赛璐璐风格、厚涂风格、月亮计划游戏《废墟图书馆》风格、大块填涂无描边笔触、强叙事性风格、涂色处厚涂，细节处细笔勾线。
# 人物设定
