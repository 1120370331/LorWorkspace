...
$output = if($OutputDirectory){[IO.Path]::GetFullPath($OutputDirectory)}else{Join-Path $repo 'output/velia-tide-mist/implementation/api'}
...
$argsList += @('BehaviourAction_Steria_VeliaTideMist.cs','FarAreaEffect_Steria_VeliaTideMist.cs','VeliaTideMistVisualController.cs','VeliaTideMistScreenFilter.cs','VeliaTideMistAudioController.cs','SkillPcmWave.cs') | ForEach-Object { Join-Path $repo ('SteriaBuild/' + $_) }
