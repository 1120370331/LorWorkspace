...
$log = Join-Path $scriptRoot "unity-build-combat.log"
$bundleName = "steria_anhier_bluewhite_combat"
...
$args = @(
    ...
    "-nographics",
    "-projectPath", $project,
    "-executeMethod", "AnhierBlueWhiteCombatBundleBuilder.BuildBundle",
    "-logFile", $log
)
...
if ($p.ExitCode -ne 0) {
    ...
    throw "Unity combat AssetBundle build failed with exit code $($p.ExitCode)."
}
...
if (!(Test-Path -LiteralPath $bundle)) {
    throw "Combat bundle was not generated in $output"
}
