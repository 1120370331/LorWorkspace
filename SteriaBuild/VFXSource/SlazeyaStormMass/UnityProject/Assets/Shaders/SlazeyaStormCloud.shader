Shader "Steria/SlazeyaStormCloud"
{
 Properties
 {
  ...
  _Collapse("Lifecycle progress",Float)=0
 ...
 SubShader
 {
  ...
  Pass
  {
   ...
   float4 _CloudCenter,_CloudInnerRadii;float _SinkTravel,_CloudHeight,_Collapse,_DebugDensity,_CloudSpinAngle,_DensityPhase;
   ...
   float4 frag(v2f i):SV_Target
   {
    ...
    float3 sourceWorld=_CloudCenter.xyz+_CloudHeight*cloudSinkInverse((i.world-_CloudCenter.xyz)/_CloudHeight,_SinkTravel,compensation,inverseGradient);
    float coreCharge=_Charge*(1-smoothstep(0.03,0.06,distance(i.world,_CloudCenter.xyz)/_CloudHeight));
    float density=sharedCloudField(sourceWorld,_CloudCenter.xyz,_CloudInnerRadii.xy,_CloudHeight,_CloudSpinAngle,_DensityPhase,_Collapse);
    ...
    float selfShadow=smoothstep(0.38,0.82,data.r)*(0.22+0.40*data.b)*bodyRole*(1-coreCharge);
    ...
    color=lerp(color,float3(0.953,0.973,1),coreCharge*0.9)+float3(0.85,0.90,1)*coreCharge;
