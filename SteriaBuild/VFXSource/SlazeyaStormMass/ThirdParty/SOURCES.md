...
Runtime diagnostics are separate from offline generation: `CloudDensitySlice.compute::CSSlice` calls the same density include as the volume shader. One box bounds the continuous ring field. View integration normally uses H*LocalScale/48 and an exact final partial step; its 1024-iteration bound increases the step for unusually long paths instead of truncating the cloud. Every nonzero density sample integrates 48 midpoint light samples of the same eroded field. Shape/detail LOD follows the world-step/voxel ratio including carrier contraction. The existing value-noise field remains only for boundary wisps. The baked shape field repeats every 2H; W4/W8/W16 sculpt .5/.25/.125H structure and can reduce body density to zero. Shared cross-section rolling and inverse wind rotation use the explicit effect clock.
