...
Check ($driver -match 'CloudInitialPosition' -and $driver -match 'CloudRoleEnd' -and $driver -match 'CollapseOffset') 'same persistent cloud identities actually collapse into a filled core'
...
Check ($volumeShader -match 'unity_OrthoParams' -and $volumeShader -match 'localDirection' -and $volumeShader -match 'step<1024' -and $volumeShader -match 'lightIndex<96' -and $volumeShader -match 'transmission<0.02' -and $densitySource -match 'return float2\(density,density\)' -and $volumeShader -notmatch '_Time|GrabPass|ZTest Always|CameraDepthTexture') 'bounded full-box volume rays, one density integral and density shadows'
Check ($volumeShader -match 'lightStep=leave/count' -and $volumeShader -match 'SampleCloudDensity\(p,lightStep\).x' -and $volumeShader -match 'clamp\(_LightSteps,48,96\)' -and $volumeShader -notmatch 'lightIndex<3|_LocalScale\*0.24') 'light integral reaches the actual box exit and samples eroded density at bounded midpoint steps'
Check ($driver -match 'MacroPose' -and $driver -match 'minimumScale' -and $builder -match 'CaptureMacroViews' -and $builder -match 'Macro full corner' -and $builder -match 'GetComponentsInChildren<Collider>') 'real visual-only macro meshes, standalone evidence and finite core corner support'
...
Check ($volumeShader -match 'fixedStep=_CloudHeight\*_LocalScale/clamp\(_StepsPerH,16,96\)' -and $volumeShader -match 'min\(fixedStep,leave-cursor\)' -and $volumeShader -match 'lightTransmission\(samplePosition,sigma\)' -and $volumeShader -notmatch 'step%4|targetLight|pow\(light,1.25\)' -and $densitySource -match 'CloudPhaseHG') 'fixed world step, final partial segment, immediate lightmarch and normalized HG'
...
Check ($driver -match 'MacroCount=1' -and $driver -match 'CloudUniformScale' -and $driver -match 'CloudContainerPose' -and $builder -match 'PrimitiveType.Cube\);var mesh' -and $volumeShader -match 'CloudBoxInterval' -and $volumeShader -match 'fixedStep=max\(fixedStep,\(leave-enter\)/1024\)' -and $cloudNoise -match '2.4\+0.7\*sin\(3\*theta\)' ) 'single box, shared finite contraction, full path coverage and continuous cross-section roll'
