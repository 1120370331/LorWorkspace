...
$wave=[IO.File]::ReadAllText((Join-Path $repo 'SteriaBuild/SkillPcmWave.cs'))
...
Check ($audio -match 'Dictionary<string, AudioClip>' -and $audio -match 'GatherGain = 0.55f' -and $audio -match 'BurstGain = 0.78f') 'cached clips and frozen gain interface'
Check ($audio -match 'expectedFrames != GatherFrames && expectedFrames != BurstFrames' -and $audio -match 'SkillPcmWave.ReadStereo44100' -and $wave -match 'MaxWaveBytes = 512 \* 1024' -and $wave -match 'rate != 44100' -and $wave -match 'blockAlign != 4' -and $wave -match 'bits != 16') 'bounded strict PCM16 stereo loader with original cue-only duration guard'
...
if(!$CandidateDirectory){$CandidateDirectory=Join-Path $repo 'preview_exports/slazeya_storm_mass/round5'}
