param([switch]$WithWaves,[string]$OutputDirectory='',[string]$CandidateDirectory='')
$ErrorActionPreference='Stop'
$repo=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../..'))
$output=if($OutputDirectory){[IO.Path]::GetFullPath($OutputDirectory)}else{Join-Path $repo 'output/storm-dawn-audio/runtime-checks/slazeya'}
New-Item -ItemType Directory -Force -Path $output | Out-Null
& (Join-Path $PSScriptRoot 'verify_slazeya_storm_audio_source.ps1') -CandidateDirectory $CandidateDirectory *> (Join-Path $output 'source-verification.log')
...
$apiArgs+=@((Join-Path $repo 'SteriaBuild/SlazeyaStormAudioController.cs'),(Join-Path $repo 'SteriaBuild/SlazeyaStormVisualController.cs'),(Join-Path $repo 'SteriaBuild/SkillPcmWave.cs'))
...
if($WithWaves){
    ...
    $harnessArgs+=@((Join-Path $repo 'SteriaBuild/SlazeyaStormAudioController.cs'),(Join-Path $repo 'SteriaBuild/SkillPcmWave.cs'),(Join-Path $PSScriptRoot 'SlazeyaStormAudioManagedChecks.cs'))
